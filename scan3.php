      ...
      <?php
        if(!empty($_POST['qrcode'])){          
          ...
          if(mysqli_num_rows($resSQL22)==1){
            $sql77= "SELECT status2 FROM abseng WHERE qr='$text'";
            $resSQL77=mysqli_query($con,$sql77);
            while ($row2 = $resSQL77->fetch_assoc()) {              
              if($row2['status2']=='1'){
                $sql33= "SELECT status3,jam_masuk FROM abseng WHERE qr='$text'";
                ...
                while ($row = $resSQL33->fetch_assoc()) {              
                  if($row['status3']=='1'){                
                    ...
                    echo "<script>Swal.fire({
                      position: 'center',
                      icon: 'warning',
                      title: 'Anda sudah masuk sesi ketiga, jam $rowg',
                    ...
                    $sql= "UPDATE abseng set status3 = '1', jam_masuk3 = '$dtg' where qr = '$text'";
